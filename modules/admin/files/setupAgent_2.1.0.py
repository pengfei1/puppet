  ...
  if retcode["exitstatus"] == 0 and retcode["log"] != None and retcode["log"] != "" and retcode["log"][0].strip() != "":
    ...
    if not isAgentPackageAlreadyInstalled(availableProjectVersion):
      ...
      if (not retcode["exitstatus"] == 0):
        ...
        cmd = ["bash", "-c", '''sed  -i 's/        distribution = platform.linux_distribution()/        distribution = platform.linux_distribution()\\n        if distribution[0].lower() == "ubuntu":\\n          distribution = (distribution[0], "12.02", distribution[2])\\n/' /usr/lib/ambari-agent/lib/ambari_commons/os_check.py''']
        ret = execOsCommand(cmd)
